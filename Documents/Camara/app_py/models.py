...
class SearchIndicateur(db.Model):
    __tablename__ = 'search_indicateurs'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    Indicateurs = db.Column(db.Text, nullable=True)
    Domaine = db.Column(db.Text, nullable=True)
    Thematique = db.Column(db.Text, nullable=True)

    def __repr__(self):
        return f"<SearchIndicateur {self.id}: {self.Indicateurs}>"


class DescriptionRegion(db.Model):
    __tablename__ = 'description_region'
    ...
    def __repr__(self):
        return f"<DescriptionRegion {self.id}: {self.Nom}>"
